...
export const matchArray = (argType: string): MatchArray | null => {
  ...
  return {
    itemType,
    size: size ? Number(size) : Number.POSITIVE_INFINITY,
    isNested,
  }
}
...
export const transformDataForArrayItem = (
  data: ContractAbiItemInput,
  index: number,
): ContractAbiItemInput => {
  const arrayMatchType = matchArray(data.type)
  const arrayMatchInternalType = data.internalType
    ? matchArray(data.internalType)
    : null
  const childrenInternalType = arrayMatchInternalType?.itemType.replaceAll(
    "struct ",
    "",
  )
