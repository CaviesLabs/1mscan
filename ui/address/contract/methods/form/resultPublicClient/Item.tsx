...
const Item = ({ abiParameter, data, mode, level = 0 }: Props) => {
  const arrayMatch = React.useMemo(
    () => matchArray(abiParameter.type),
    [abiParameter.type],
  )
  if (arrayMatch) {
    if (!Array.isArray(data)) {
      if (
        arrayMatch.itemType === "tuple" &&
        mode === "preview" &&
        "components" in abiParameter
      ) {
        const previewData = Object.fromEntries(
          abiParameter.components.map(({ name }) => [name ?? "", undefined]),
        )
        return (
          <ItemArray
            abiParameter={abiParameter}
      ...
      return (
        <ItemPrimitive
          abiParameter={abiParameter}
          data={data || (mode === "preview" ? undefined : "[ ]")}
          level={level}
        />
      )
    }
