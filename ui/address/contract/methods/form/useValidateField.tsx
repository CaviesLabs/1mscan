...
export default function useValidateField({
  isOptional,
  argType,
  argTypeMatchInt,
}: Params) {
    ...
    (value: string | boolean | undefined) => {
      ...
      if (argTypeMatchInt) {
        ...
        if (valueBi > argTypeMatchInt.max || valueBi < argTypeMatchInt.min) {
          const lowerBoundary = argTypeMatchInt.isUnsigned
            ? "0"
            : `-1 * 2 ^ ${Number(argTypeMatchInt.power) - 1}`
          const upperBoundary = argTypeMatchInt.isUnsigned
            ? `2 ^ ${argTypeMatchInt.power} - 1`
            : `2 ^ ${Number(argTypeMatchInt.power) - 1} - 1`
      ...
      if (bytesMatch) {
        ...
        if (length) {
          ...
          return valueLengthInBytes !== Number(length)
            ? `Value should be ${length} bytes in length`
            : true
        }
