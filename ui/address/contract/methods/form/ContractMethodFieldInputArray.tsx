...
interface Props
  extends Pick<AccordionProps, "onAddClick" | "onRemoveClick" | "index"> {
...
const ContractMethodFieldInputArray = ({
  ...
  const hasFixedSize =
    arrayMatch !== null && arrayMatch.size !== Number.POSITIVE_INFINITY
  ...
  const handleRemoveButtonClick = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      if (itemIndex) {
        setRegisteredIndices((prev) =>
          prev.filter((index) => index !== Number(itemIndex)),
        )
      }
    },
    [],
    ...
    return (
      <>
        {registeredIndices.map((registeredIndex, index) => {
          ...
          const itemIsInvalid = fieldsWithErrors.some((field) =>
            field.startsWith(itemBasePath),
          )
