...
export const isReadMethod = (
  method: Abi[number],
): method is SmartContractMethodRead =>
  method.type === "function" &&
  (method.constant ||
    method.stateMutability === "view" ||
    method.stateMutability === "pure")

export const isWriteMethod = (
  method: Abi[number],
): method is SmartContractMethodWrite =>
  (method.type === "function" ||
    method.type === "fallback" ||
    method.type === "receive") &&
  !isReadMethod(method)

const enrichWithMethodId = (
  method: AbiFunction,
): SmartContractMethodCustomFields => {
  try {
    return {
...
export function divideAbiIntoMethodTypes(abi: Abi): DividedAbi {
  return {
    read: abi.filter(isReadMethod).map((method) => ({
      ...method,
      ...enrichWithMethodId(method),
    })),
    write: abi.filter(isWriteMethod).map((method) => {
