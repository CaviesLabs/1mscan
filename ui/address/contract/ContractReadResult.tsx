...
const ContractReadResult = ({ item, result, onSettle }: Props) => {
  ...
  if ("status" in result) {
    return (
      <ContractReadResultError>{result.statusText}</ContractReadResultError>
    )
  }
  ...
  if (result.is_error) {
    if ("error" in result.result) {
      return (
        <ContractReadResultError>{result.result.error}</ContractReadResultError>
      )
    }
    ...
    if ("raw" in result.result) {
      return (
        <ContractReadResultError>{`Revert reason: ${hexToUtf8(
          result.result.raw,
        )}`}</ContractReadResultError>
      )
    }
    ...
    return (
      <ContractReadResultError>Something went wrong.</ContractReadResultError>
    )
  }
