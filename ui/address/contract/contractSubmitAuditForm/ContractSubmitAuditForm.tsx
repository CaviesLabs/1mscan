...
const ContractSubmitAuditForm = ({ address, onSuccess }: Props) => {
  ...
  const onFormSubmit: SubmitHandler<Inputs> = React.useCallback(
    async (data) => {
      try {
        await apiFetch<"contract_security_audits">("contract_security_audits", {
        ...
        if (errorMap && Object.keys(errorMap).length) {
          ;(Object.keys(errorMap) as Array<keyof Inputs>).forEach(
            (errorField) => {
              setError(errorField, {
                type: "custom",
                message: errorMap[errorField].join(", "),
              })
            },
          )
        } else {
          toast({
            ...
            title: "Error",
            description:
              (_error as ResourceError<{ message: string }>)?.payload
