...
// Define the ICosmos type based on the cosmos object, omitting the "ClientFactory" key
type ICosmos = Omit<typeof cosmos, "ClientFactory">
...
export type UseCosmosMsgExecute = <
...
export type UseCosmosMsgClientReturnType = IUseCurrentChain & {
  execute: UseCosmosMsgExecute
}

export const useCosmosMsgClient = () => {
  const cosmosChain = useCosmosChain()

  const execute = useCallback<UseCosmosMsgExecute>(
      ...
      return new Promise((resolve) => resolve(0))
        .then(async () => {
          ...
          const module = await import("@sei-js/proto")
            .then((module) => module.cosmos)
            .then((cosmos) => get(cosmos, path) as any)

          // const module = get(cosmos, path) as any;
  ...
  return useMemo(() => {
    ...
    return cosmosChain as unknown as UseCosmosMsgClientReturnType
  }, [cosmosChain, execute])
}
