export default function getFilePathParts(
  path: string,
...
function getFolderName(
  ...
  const fileName = chunks[chunks.length - 1]
  const otherTabsPathChunks = tabsPathChunks.filter(
    (item) => item.join("/") !== chunks.join("/"),
  )
  const tabsWithSameFileName = otherTabsPathChunks.filter(
    (tabChunks) => tabChunks[tabChunks.length - 1] === fileName,
  )
  ...
  for (let index = 3; index <= chunks.length; index++) {
    ...
    const subFolderNames = tabsWithSameFileName.map(
      (tab) => tab[tab.length - index],
    )
