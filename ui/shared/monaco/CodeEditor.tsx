...
import { Box, Center, Flex, useColorMode, useToken } from "@chakra-ui/react"
...
const CodeEditor = ({
  ...
  const handleSelectFile = React.useCallback(
    (index: number, lineNumber?: number) => {
      ...
      if (lineNumber !== undefined && !Object.is(lineNumber, Number.NaN)) {
        window.setTimeout(() => {
    ...
    (path: string, _isActive?: boolean) => {
      setTabs((prev) => {
        if (prev.length > 1) {
          ...
          const isActive =
            _isActive !== undefined ? _isActive : data[index].file_path === path
  ...
  if (data.length === 1) {
    ...
    return (
      ...
      >
        <ErrorBoundary fallbackRender={renderErrorScreen}>
          <MonacoEditor
