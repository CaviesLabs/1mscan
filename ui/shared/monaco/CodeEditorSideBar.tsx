...
import { cmd, shift } from "lib/html-entities"
...
const CodeEditorSideBar = ({
  ...
  const handleScrollThrottled = React.useRef(
    _throttle((event: React.SyntheticEvent) => {
      ...
      >
        ...
        >
          <TabList
            ...
            borderTopRightRadius="md"
          >
            <Tab {...tabProps} title={`File explorer (${shift + cmd}E)`}>
              Explorer
            </Tab>
            <Tab {...tabProps} title={`Search in files (${shift + cmd}F)`}>
              Search
            </Tab>
            {actionBarRenderer?.()}
          </TabList>
