...
const CodeEditorFileTree = ({
  ...
  return (
    <Accordion
      allowMultiple
      defaultIndex={isCollapsed ? undefined : tree.map((item, index) => index)}
      reduceMotion
    >
      {tree.map((leaf, index) => {
        const leafName = (
              ...
              {({ isExpanded }) => (
                <>
                  <AccordionButton
                    ...
                    pl={`${8 + 8 * level}px`}
                    _hover={{
                      bgColor: themeColors["custom.list.hoverBackground"],
                    }}
                    fontSize="13px"
                    lineHeight="22px"
                    h="22px"
        ...
        return (
          <AccordionItem
            ...
            _hover={{
              bgColor:
                selectedFile === leaf.file_path
                  ? themeColors["list.inactiveSelectionBackground"]
                  : themeColors["custom.list.hoverBackground"],
            }}
            bgColor={
              selectedFile === leaf.file_path
                ? themeColors["list.inactiveSelectionBackground"]
