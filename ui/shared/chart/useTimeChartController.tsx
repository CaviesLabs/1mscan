...
export default function useTimeChartController({
  ...
  const axisParams = React.useMemo(
    () => getAxisParams(data, axesConfig),
    [data, axesConfig, rect?.width],
  )

  const chartMargin = React.useMemo(() => {
    const exceedingDigits =
      (axisParams.y.labelFormatParams.maximumSignificantDigits ??
        DEFAULT_MAXIMUM_SIGNIFICANT_DIGITS) - DEFAULT_MAXIMUM_SIGNIFICANT_DIGITS
  ...
  return React.useMemo(() => {
    ...
    axisParams.x.tickFormatter,
    axisParams.y.tickFormatter,
    chartMargin,
    innerHeight,
    innerWidth,
    rect,
