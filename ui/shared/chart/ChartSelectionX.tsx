...
const ChartSelectionX = ({
  ...
  const handleSelect = React.useCallback(
    (x0: number, x1: number) => {
      ...
      if (
        Math.abs(moment(startDate).diff(endDate, "day")) > SELECTION_THRESHOLD
      ) {
        onSelect([
          moment.min(moment(startDate), moment(endDate))!.toDate(),
          moment.max(moment(startDate), moment(endDate))!.toDate(),
        ])
      }
    },
    [onSelect, scale],
  ...
  React.useEffect(() => {
    ...
    anchorD3
      ...
      .on("touchmove.selectionX", (event: TouchEvent) => {
        if (isActive.current) {
          ...
          if (
            pointers.length === 2 &&
            Math.abs(pointers[0][0] - pointers[1][0]) > 5
          ) {
    ...
    d3.select("body").on("mouseup.selectionX", (event) => {
      const isOutside =
        startX.current !== undefined && event.target !== anchorD3.node()
      if (isOutside) {
        handelMouseUp()
      }
    })
  ...
  return (
    <g className="ChartSelectionX" ref={ref} opacity={0}>
      <rect
        className="ChartSelectionX__rect"
        width={0}
        height={height}
        fill="rgba(66, 153, 225, 0.1)"
      />
      <line
        className="ChartSelectionX__line ChartSelectionX__line_left"
        x1={0}
        x2={0}
        y1={0}
        y2={height}
      ...
      />
      <line
        className="ChartSelectionX__line ChartSelectionX__line_right"
        x1={0}
        x2={0}
        y1={0}
        y2={height}
