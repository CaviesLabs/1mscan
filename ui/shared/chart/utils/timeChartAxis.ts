...
import { MONTH, WEEK, YEAR } from "lib/consts"
...
export function getAxisParams(data: TimeChartData, axesConfig?: AxesConfig) {
  const { labelFormatParams: labelFormatParamsY, scale: yScale } =
    getAxisParamsY(data, axesConfig?.y)
...
const tickFormatterX =
  (axis: d3.Axis<d3.NumberValue>) => (d: d3.AxisDomain) => {
...
const getTickFormatterY =
  (params: Intl.NumberFormatOptions) => () => (d: d3.AxisDomain) => {
    const num = Number(d)
...
function getYLabelFormatParams(
  ...
  const uniqTicksStr = _unique(
    ticks.map((tick) => tick.toLocaleString(undefined, params)),
  )

  if (
    uniqTicksStr.length === ticks.length ||
    maximumSignificantDigits === MAXIMUM_SIGNIFICANT_DIGITS_LIMIT
  ) {
    return params
  }
