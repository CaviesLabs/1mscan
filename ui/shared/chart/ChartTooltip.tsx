...
const ChartTooltip = ({
  ...
  const markerBorderColor = useToken(
    "colors",
  ...
  const drawContent = React.useCallback(
    (x: number, y: number) => {
      ...
      const dateLabel = data[0].items.find(
      ...
      d3.select(ref.current)
        ...
        .attr("transform", (cur, i) => {
          ...
          const d = (() => {
            ...
            return xDate.getTime() - d0.date.getTime() >
    ...
    <g ref={ref} opacity={0} {...props}>
      ...
      <g className="ChartTooltip__content">
        <rect
          ...
          height={
            2 * PADDING +
            (data.length + 1) * TEXT_LINE_HEIGHT +
            data.length * LINE_SPACE
          }
        />
        ...
        {data.map(({ name }, index) => (
          <g
            ...
            transform={`translate(${PADDING},${
              PADDING + (index + 1) * (LINE_SPACE + TEXT_LINE_HEIGHT)
            })`}
          >
            <text
              className="ChartTooltip__contentTitle"
