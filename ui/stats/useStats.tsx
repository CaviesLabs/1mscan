...
import { ethToSeiObject } from "lib/utils/format"
import { STATS_CHARTS } from "stubs/stats"

function isSectionMatches(
...
export default function useStats() {
  ...
  const displayedCharts = React.useMemo(() => {
    return data?.sections
      ?.map((section) => {
        const charts = section.charts.filter(
          (chart) =>
            isSectionMatches(section, currentSection) &&
            isChartNameMatches(debouncedFilterQuery, chart),
        )

        return {
          ...section,
          charts: ethToSeiObject(charts),
        }
      })
