...
import NATIVECountersItem from "./NATIVECountersItem"

interface Props {
  hash: string
}

const FactoryTokenDetails = ({ hash }: Props) => {
      ...
      >
        ...
        {associationTokenData && (
          <InfoItem
            ...
            hint={getLanguage(
              "token.the_pointer_address_is_a_smart_contract_where_the_tokens_are_managed_on_the_parallel_blockchain_by_calling_from_its_factory_token",
            )}
            titleProps={{
              width: {
                base: "full",
        ...
        {token?.denom_trace && (
          <InfoItem
            ...
            isTruncated
          >
            <SkeletonText isLoading={isLoading} isTruncated>
              {token.denom_trace}
            </SkeletonText>
          </InfoItem>
        )}
...
export default memo(FactoryTokenDetails, (prev, next) => {
