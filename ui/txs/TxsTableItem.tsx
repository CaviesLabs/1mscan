...
import TxTypeMethod from "./TxTypeMethod"
...
const TxsTableItem = ({ item, isLoading, osType }: Props) => {
    ...
    <Tr role="group">
      <Td>
        <VStack alignItems="stretch" spacing={1} overflow="hidden">
          <TxEntityV2 hash={item.hash} isLoading={isLoading} fallback="-" />
          {item.timestamp && (
            <Skeleton
...
export default memo(TxsTableItem, (prev, next) => {
